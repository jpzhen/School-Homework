...
//void serverEarlyRetrans( UdpSocket &sock, const int max, int message[], int windowSize, bool congestion );
...
int main(int argc, char *argv[]) 
{
    ...
    cerr << "   3: sliding windows test" << endl;
    cerr << "   4: Sliding window with drop test" << endl;
    cerr << "--> ";
    cin >> testNumber;
    ...
    if (myPart == CLIENT) 
    {
        ...
        switch (testNumber) 
        {
            ...
            case 3: 
                ...
                {
                    timer.start();                                        // start timer
                    retransmits = ans.clientSlidingWindow(sock, MAX, message, windowSize); // actual test
            ...
            case 4:
                ...
                {
                    for (int dropChance = 0; dropChance <= 10; dropChance++) 
                    {
                        timer.start();                                        // start timer
                        retransmits = ans.clientSlidingWindow(sock, MAX, message, windowSize); // actual test
                        cerr << "Drop chance = ";
    ...
    if (myPart == SERVER) 
    {
        switch (testNumber)
        {
            ...
            case 3: 
                for (int windowSize = 1; windowSize <= MAXWIN; windowSize++) 
                {
                    ans.serverEarlyRetrans(sock, MAX, message, windowSize, 0);
                }
                break;
            case 4:
                for (int windowSize = 1; windowSize <= MAXWIN; windowSize += 29) 
                {
                    for (int dropChance = 0; dropChance <= 10; dropChance++) 
                    {
                        ans.serverEarlyRetrans(sock, MAX, message, windowSize, dropChance);
                    }
                }
                break;
            default:
